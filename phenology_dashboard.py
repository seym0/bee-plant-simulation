...
def get_base_simulation_df() -> pd.DataFrame:
    """Базовый ряд 1950–2100 (D_T по умолчанию в модуле симуляции)."""
    return pm.load_simulation().reset_index(drop=True)


@st.cache_data(show_spinner=False)
def get_spring_x() -> np.ndarray:
    return np.linspace(pm.SPRING_DAY_MIN, pm.SPRING_DAY_MAX, pm.SPRING_RESOLUTION)


@st.cache_data(show_spinner=False)
def cached_overlap_figure(year_idx: int) -> go.Figure:
    df = get_base_simulation_df()
    return figure_overlap_year_with_species_icons(df.iloc[year_idx], get_spring_x())


@st.cache_data(show_spinner=False)
def cached_population_timeseries_figure() -> go.Figure:
    return pm.figure_population_timeseries(get_base_simulation_df())


@st.cache_data(show_spinner=False)
def cached_dt_block(d_t: float) -> tuple[go.Figure, int | None, int | None]:
    """Симуляция при D_T, Plotly-график и годы коллапса популяций."""
    d_t = round(float(d_t), 3)
    df_dt = podt.run_simulation_for_dt(d_t)
    fig = podt.figure_overlap_results_plotly(df_dt, d_t)
    return (
        fig,
        first_year_at_or_below(df_dt, "Популяция_Пчел"),
        first_year_at_or_below(df_dt, "Популяция_Растений"),
    )


@st.cache_data(show_spinner="Подготовка модели…")
def warm_dashboard_cache() -> None:
    """Прогрев кэша при первом открытии (один раз на инстанс Cloud)."""
    get_base_simulation_df()
    podt.fixed_phenology_ylim()
    cached_overlap_figure(0)
    cached_population_timeseries_figure()
    cached_dt_block(podt.DT_DEFAULT)
...
def main() -> None:
    ...
    df_year = get_base_simulation_df()
    ...
    st.plotly_chart(cached_overlap_figure(year_idx), use_container_width=True)
...
""",
    ...
    fig_dt, y_bees, y_plants = cached_dt_block(d_t)
    ...
    st.plotly_chart(cached_population_timeseries_figure(), use_container_width=True)
