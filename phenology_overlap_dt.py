...
@st.cache_data(show_spinner=False)
def figure_overlap_results_plotly_cached(d_t: float) -> go.Figure:
    """Кэшированная обёртка: симуляция + Plotly-фигура для дашборда и Cloud."""
    d_t = round(float(d_t), 3)
    df = run_simulation_for_dt(d_t)
    return figure_overlap_results_plotly(df, d_t)
